...
if (isset($_POST['Category'])) {
    ...
    if (count($Cat) > 0) {
        $categoryConditions = array_map(function ($category) {
            return "LOWER(SHOP_REQUEST.CATEGORY) = LOWER('" . $category . "')";
        }, $Cat);

        $categoryFilter = implode(' OR ', $categoryConditions);
        ...
        if (isset($_GET['search'])) {
            $sql = "SELECT * FROM PRODUCT 
                    ...
                    WHERE PRODUCT.IS_DISABLED = 'false' 
                    AND (LOWER(PRODUCT.PRODUCT_NAME) LIKE LOWER('%$search%')) 
                    AND ($categoryFilter)";
        } else {
            $sql = "SELECT * FROM PRODUCT 
                    INNER JOIN SHOP ON PRODUCT.SHOP_ID = SHOP.SHOP_ID 
